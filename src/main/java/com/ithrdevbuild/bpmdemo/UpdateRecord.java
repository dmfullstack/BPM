...
public class UpdateRecord implements JavaDelegate {
	public static void main(String args[]) {
		...
		UpdateRecord cr = new UpdateRecord();
	...
	public void execute(DelegateExecution delegateExecution) throws Exception {
		...
		if (readContacts.size() >0){
			...
			bulkUpdate(client, readContacts,contact.getPhoneWork());
			
		} else {
			//contactId = createRecord(client, contact);
